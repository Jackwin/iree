...
static iree_status_t iree_hal_vmvx_executable_verify_entry_point(
  ...
  if (!iree_string_view_equal(signature.calling_convention,
                              iree_make_cstring_view("0rriii"))) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "executable entry point does not match the expected calling "
        "convention; expected '0rriii' but got '%.*s'",
...
static iree_status_t iree_hal_vmvx_executable_issue_call(
  ...
  //
  // Signature: entry_fn(interface, scratchpad, x, y, z)
  //
  ...
  struct {
    ...
    uint32_t workgroup_z;
      ...
      .workgroup_x = dispatch_state->workgroup_count.x,
      .workgroup_y = dispatch_state->workgroup_count.y,
      .workgroup_z = dispatch_state->workgroup_count.z,
  };
